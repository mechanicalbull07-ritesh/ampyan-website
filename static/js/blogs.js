(function(){
  ...
  if(editor){
    ...
    function safeHttpsUrl(value){try{var parsed=new URL(value);return parsed.protocol==="https:"?parsed.href:"";}catch(error){return"";}}
    ...
    function renderEditor(){sync();list.textContent="";blocks.forEach(function(block,index){var card=document.createElement("section");card.className="blog-block-card";var heading=document.createElement("h3");heading.textContent=(index+1)+". "+String(block.type).replaceAll("_"," ");card.appendChild(heading);
      ...
      if(block.type==="image"){var previewUrl=safeHttpsUrl(block.data.url);if(previewUrl){var preview=document.createElement("img");preview.className="blog-editor-image-preview";preview.src=previewUrl;preview.alt=block.data.alt_text||"Image block preview";preview.loading="lazy";card.appendChild(preview);}card.appendChild(field("Alt text",block.data.alt_text,function(value){block.data.alt_text=value;}));card.appendChild(field("Caption (optional)",block.data.caption,function(value){block.data.caption=value;}));}
    ...
    async function uploadImage(input,status){var file=input.files[0];if(!file){throw new Error("Choose an image first.");}if(!["image/png","image/jpeg","image/webp"].includes(file.type)){throw new Error("Choose a PNG, JPEG or WebP image.");}if(file.size>8*1024*1024){throw new Error("Image must be 8 MB or smaller.");}var token=editor.querySelector('[name="csrf_token"]').value,body=new FormData();body.append("image",file);body.append("csrf_token",token);status.textContent="Uploading…";var response=await fetch("/blogs/media",{method:"POST",body:body,credentials:"same-origin",headers:{"X-CSRFToken":token}});var result=await response.json();if(!response.ok||!result.success)throw new Error(result.message||"Upload failed.");return{file:file,media:result.media};}
    editor.querySelector("[data-upload-media]").addEventListener("click",async function(){var input=editor.querySelector("[data-media-file]"),status=editor.querySelector("[data-media-status]");try{var uploaded=await uploadImage(input,status);blocks.push({id:blockId(),type:"image",order:blocks.length,data:{url:uploaded.media.url,alt_text:uploaded.file.name}});dirty=true;renderEditor();status.textContent="Image added.";}catch(error){status.textContent=error.message||"Image upload failed.";}});
    var coverUrl=editor.querySelector("[data-cover-url]"),coverPreview=editor.querySelector("[data-cover-preview]");
    function renderCover(){coverPreview.textContent="";var previewUrl=safeHttpsUrl(coverUrl.value);if(previewUrl){var image=document.createElement("img");image.src=previewUrl;image.alt="Cover image preview";coverPreview.appendChild(image);}}
    editor.querySelector("[data-upload-cover]").addEventListener("click",async function(){var input=editor.querySelector("[data-cover-file]"),status=editor.querySelector("[data-cover-status]");try{var uploaded=await uploadImage(input,status);coverUrl.value=uploaded.media.url;dirty=true;renderCover();status.textContent="Cover image uploaded.";}catch(error){status.textContent=error.message||"Cover image upload failed.";}});
    renderCover();
    renderEditor();
  }
})();
